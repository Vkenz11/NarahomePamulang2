...
import { PROPERTY_CONFIG, injectSEOMetadata } from "./config/propertyData";
...
function useFocusTrap(isOpen: boolean, ref: React.RefObject<HTMLElement | null>, onClose: () => void) {
  useEffect(() => {
    ...
    // Small timeout to allow animation/render to finish and browser to process focus safely
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Tab") {
        const currentElements = Array.from(modalElement.querySelectorAll(focusableSelectors)) as HTMLElement[];
...
export default function App() {
  ...
  // Refs for modals to handle focus trap
  const videoModalRef = useRef<HTMLDivElement>(null);
  const bookingModalRef = useRef<HTMLDivElement>(null);
  const exitIntentModalRef = useRef<HTMLDivElement>(null);
  const articleModalRef = useRef<HTMLDivElement>(null);
  const unitModalRef = useRef<HTMLDivElement>(null);

  // Hook focus traps for each modal
  useFocusTrap(showVideoModal, videoModalRef, () => setShowVideoModal(false));
  useFocusTrap(showBookingModal, bookingModalRef, () => setShowBookingModal(false));
  useFocusTrap(showExitIntent, exitIntentModalRef, () => setShowExitIntent(false));
  useFocusTrap(!!selectedArticle, articleModalRef, () => setSelectedArticle(null));
  useFocusTrap(!!selectedUnit, unitModalRef, () => setSelectedUnit(null));

  // Dynamic SEO metadata injection
  useEffect(() => {
    if (selectedArticle) {
      injectSEOMetadata("articles", {
        title: `${selectedArticle.title} | Nara Home Pamulang`,
        description: selectedArticle.summary || selectedArticle.title,
        keywords: selectedArticle.keywords || []
      });
    } else {
      injectSEOMetadata(activeSection);
    }
  }, [activeSection, selectedArticle]);

  // Dynamic WhatsApp text helper with GTM Tracking
  const triggerWhatsApp = (customText?: string) => {
    if (typeof window !== "undefined") {
      ...
      (window as any).dataLayer.push({
        ...
        event_value: customText || 'default_whatsapp_message'
      });
    }
  ...
  // Site visit booking submit handler with GTM Tracking
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    if (typeof window !== "undefined") {
      ...
      (window as any).dataLayer.push({
        ...
        event_category: 'lead',
        event_action: 'submit',
        event_label: 'booking_form_submit_success',
        booking_name: bookingName,
        booking_date: bookingDate || 'flexible'
      });
    }
  ...
  const roomDetails: Record<string, { title: string; size: string; desc: string }> = {
    ...
    bed1st: { title: "Kamar 1 (Lantai 1)", size: "2.5m x 2.5m", desc: "Kamar tidur serbaguna di lantai 1 dengan pencahayaan dan ventilasi optimal." },
    master: { title: "Master Bedroom", size: "4.0m x 3.2m", desc: "Kamar tidur utama super luas di lantai 2, dengan jendela besar berpemandangan asri." },
    bed2: { title: "Kamar Tidur Anak", size: "3.1m x 2.5m", desc: "Kamar anak bernuansa hangat dengan pencahayaan alami optimal untuk tumbuh kembang buah hati." },
    bath2: { title: "Kamar Mandi Lantai 2", size: "2.5m x 1.5m", desc: "Akses mudah dari seluruh kamar tidur lantai 2, sanitari premium, ventilasi lancar." },
  };
    ...
    <div className="relative min-h-screen flex flex-col font-sans overflow-x-hidden selection:bg-accent selection:text-white">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {floorPlanLevel === "1st" ? (
                        <>
                          ...
                          <button
                            onClick={() => setSelectedRoom(selectedRoom === "bed1st" ? null : "bed1st")}
                            className={`text-[10px] uppercase font-bold tracking-wider px-3 py-1.5 rounded border transition-all ${
                              selectedRoom === "bed1st" 
                                ? "bg-accent text-white border-accent shadow-sm" 
                                : "bg-white dark:bg-slate-800 text-slate-700 dark:text-gray-200 border-gray-200 dark:border-slate-700 hover:border-accent"
        ...
        {selectedUnit && (
          <div className="fixed inset-0 z-55 bg-black/75 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-gray-100 dark:border-slate-800 w-full max-w-4xl max-h-[90vh] flex flex-col focus:outline-none"
            >
              <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-800 bg-cream/30 dark:bg-slate-950/40">
                <div>
                  <h3 id="unit-modal-title" className="font-serif text-lg font-bold text-slate-900 dark:text-white flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 text-slate-400 dark:text-gray-400 hover:text-slate-600 dark:hover:text-white rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-accent"
      ...
      {showVideoModal && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
          <div
            ...
            className="bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-800 w-full max-w-3xl focus:outline-none"
          >
            <div className="flex items-center justify-between p-4 border-b border-slate-800 text-white">
              <h3 id="video-modal-title" className="font-serif font-bold text-sm sm:text-base">HD Video Virtual Tour - Nara Home Pamulang</h3>
              <button
                onClick={() => setShowVideoModal(false)}
                className="text-slate-400 hover:text-white p-1 rounded-full hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-accent"
                aria-label="Tutup Video Virtual Tour"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
      ...
      {showBookingModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div
            ...
            className="bg-white rounded-2xl overflow-hidden shadow-2xl border border-gray-100 w-full max-w-md text-left p-6 relative focus:outline-none"
          >
            <button
              onClick={() => setShowBookingModal(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 p-1.5 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-accent"
              aria-label="Tutup Form Pemesanan Survey"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="mb-6">
              ...
              <h3 id="booking-modal-title" className="font-serif text-2xl font-bold text-slate-900 mt-2">Jadwalkan Kunjungan Show Unit</h3>
      ...
      {showExitIntent && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center p-4">
          <div
            ref={exitIntentModalRef}
            role="dialog"
            aria-modal="true"
            aria-labelledby="exit-modal-title"
            className="bg-cream rounded-2xl overflow-hidden shadow-2xl border border-white w-full max-w-lg text-left relative p-6 sm:p-8 animate-fade-in focus:outline-none"
          >
            <button
              onClick={() => setShowExitIntent(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 p-1.5 rounded-full hover:bg-gray-150 focus:outline-none focus:ring-2 focus:ring-accent"
            ...
            <div className="space-y-4">
              ...
              <h3 id="exit-modal-title" className="font-serif text-2xl sm:text-3xl font-bold text-slate-950 leading-tight">
                Tunggu! Jangan Lewatkan Bonus Spesial Pertama Anda
              </h3>

              <p className="text-slate-600 text-xs sm:text-sm leading-relaxed font-light">
                Hanya bagi pengunjung yang menghubungi kami hari ini, dapatkan voucher <strong className="text-primary font-bold">Subsidi Biaya KPR hingga 20 Juta*</strong>, gratis kanopi carport modern, dan kemudahan booking fee!
              </p>
              ...
              <div className="bg-white p-4 rounded-xl border border-gray-150 space-y-2 text-xs">
                <div className="flex items-center space-x-2 text-slate-700">
                  ...
                  <span>Free Biaya Surat dan Akad</span>
                </div>
                <div className="flex items-center space-x-2 text-slate-700">
                  <Check className="h-4 w-4 text-accent" />
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div
            ...
            className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-gray-100 dark:border-slate-800 w-full max-w-2xl text-left relative max-h-[90vh] flex flex-col focus:outline-none"
          >
            ...
            <div className="p-6 border-b border-gray-100 dark:border-slate-800/80 flex items-center justify-between sticky top-0 bg-white dark:bg-slate-900 z-10">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-700 dark:hover:text-white p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-accent"
            ...
            <div className="overflow-y-auto p-6 space-y-6 flex-1">
              ...
              <div className="space-y-3">
                ...
                <h2 id="article-modal-title" className="font-serif text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white leading-tight">
